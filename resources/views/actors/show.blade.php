...
@section('content')
<div id="actor-info" class="border-b border-gray-800">
    <div class="container mx-auto px-4 py-16 flex flex-col md:flex-row">
        <div class="flex-none">
            <img src="{{ $actor['profile_url'] }}" alt="{{ $actor['slug'] }}" class="w-76">
        ...
        <div class="md:ml-24">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-8">
                @foreach ($popularMovies as $movie)
                    <div class="mt-4">
                        <a href="{{ $movie['link'] }}">
                            <img src="{{ $movie['poster_url'] }}" alt="{{ $movie['slug'] }}" class="hover:opacity-75 transition ease-in-out duration-150">
                        </a>
                        <a href="{{ $movie['link'] }}" class="text-sm leading-normal block text-gray-400 hover:text-white mt-1">
                            {{ $movie['title'] }}
                        </a>
                    </div>
                @endforeach
    ...
    <div class="container mx-auto px-4 py-16">
        ...
        <ul class="list-disc leading-loose pl-5 mt-8">
            @foreach ($credits as $credit)
                <li>{{ $credit['release_year'] }} &middot; <strong><a href="{{ $credit['page_url'] }}" class="hover:underline">{{ $credit['title'] }}</a></strong> as {{ $credit['character'] }}</li>
            @endforeach
        </ul>
    </div>
</div>
